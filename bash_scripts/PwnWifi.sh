...
function helpPanel(){
	echo -e "\n${yellowColour}[*]${endColour}${grayColour} Uso: ./PwnWifi.sh${endColour}"
...
function startAttack(){
		...
		ifconfig wlan0mon up; killall dhclient wpa_supplicant 2>/dev/null

		echo -e "${yellowColour}[*]${endColour}${grayColour} Nueva dirección MAC asignada ${endColour}${purpleColour}[${endColour}${blueColour}$(macchanger -s wlan0mon | grep -i current | xargs | cut -d ' ' -f '3-100')${endColour}${purpleColour}]${endColour}"
