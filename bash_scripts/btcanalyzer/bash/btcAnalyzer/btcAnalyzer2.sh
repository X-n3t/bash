...
function helpPanel(){
	echo -e "\n${redColour}\t[!] Uso: ./btcAnalyzer2${endColour}  (Author: S4vitar - Forked: X)${yellowColour}   *** Arch Based-Distribution Version ***${endColour}"
	echo -ne "\t";for i in $(seq 1 140); do echo -ne "${redColour}-"; done; echo -ne "${endColour}"
...
function inspectAddress(){
	address_hash=$1
	echo "Transacciones realizadas_Cantidad total recibida (BTC)_Cantidad total enviada (BTC)_Saldo actual en la cuenta (BTC)" > address.tmp
	...
	sed '1iTransacciones realizadas_Cantidad total recibida (USD)_Cantidad total enviada (USD)_Saldo actual en la cuenta (USD)' -i address2.tmp
