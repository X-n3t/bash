...
function helpPanel(){
	echo -e "\n${redColour}[!] Uso: ./btcAnalyzer${endColour}  (Author: S4vitar - Forked: X)"
	for i in $(seq 1 170); do echo -ne "${redColour}-"; done; echo -ne "${endColour}"
	echo -e "\n\n\t${grayColour}[-e]${endColour}${yellowColour} Modo exploración:${endColour}\n"
	echo -e "\t\t${purpleColour}unconfirmed_transactions:${endColour}${greenColour}\t Listar transacciones no confirmadas${endColour}"
	echo -e "\t\t${redColour}[-n]${endColour}${yellowColour} Limitar el número de resultados${endColour}${blueColour} (Ejemplo: ./btcAnalyzer -e unconfirmed_transactions -n 10) ${turquoiseColour} *** -n 100 por defecto ***${endColour}${endColour}"
	echo -e "\n\t\t${purpleColour}inspect:${endColour}${greenColour}:\t\t\t Inspeccionar un hash de transacción${endColour}"
	echo -e "\t\t${redColour}[-i]${endColour}${yellowColour} Proporcionar el identificador de la transacción${blueColour} (Ejemplo: ./btcAnalyzer -e inspect -i ac03c17c75686e51db5967a65b2c2b2813b128c8fd826d56b93dc0c2bc4fdfe3)${endColour}"
	echo -e "\n\t\t${purpleColour}address:${endColour}${greenColour}\t\t\t Inspeccionar una transacción de dirección${endColour}"
	echo -e "\t\t${redColour}[-a]${endColour}${yellowColour} Proporcionar una dirección de transacción${endColour}${blueColour} (Ejemplo: ./btcAnalyzer -e address -a 37jNb7LEpk7g33GHjWvBZg9xrPh6pofMyX)${endColour}"
...
function inspectTransaction(){
	...
	while [ "$(cat total_entrada_salida.tmp | wc -l)" == "1" ]; do
		curl -s "${inspect_transaction_url}${inspect_transaction_hash}" | html2text | grep -E -A 1 "Entrada total|Salida total" | grep -v -E "Entrada total|Salida total|\--" | xargs | tr ' ' '_' | sed 's/_BTC/ BTC/g' >> total_entrada_salida.tmp
	done
	...
	while [ "$(cat entradas.tmp | wc -l)" == "1" ]; do
		curl -s "${inspect_transaction_url}${inspect_transaction_hash}"| html2text | grep -A 500 "Entradas" | grep -B 500 "Salidas" | grep "Direcci" -A 3 | grep -v -E "Direcci|Valor|\--" | awk 'NR%2{printf "%s ",$0;next;}1' | awk '{print $1 "_" $2 " " $3}' >> entradas.tmp
	done
	...
	while [ "$(cat salidas.tmp | wc -l)" == "1" ]; do
		curl -s "${inspect_transaction_url}${inspect_transaction_hash}"| html2text | grep -A 500 "Salidas" | grep -B 500 "Lo has pensado" | grep "Direcci" -A 3 | grep -v -E "Direcci|Valor|\--" | awk 'NR%2{printf "%s ",$0;next;}1' | awk '{print $1 "_" $2 " " $3}' >> salidas.tmp
	done
...
function inspectAddress(){
	...
	curl -s "${inspect_address_url}${address_hash}" | html2text | grep -A 1 -E "Transacciones|Total Recibidas|Cantidad total enviada|Saldo final" | head -n -2 | grep -v -E "Transacciones|Total Recibidas|Cantidad total enviada|Saldo final" | xargs | tr ' ' '_' | sed 's/_BTC/ BTC/g' >> address.tmp
	...
	bitcoin_value=$(curl -s "https://es.cointelegraph.com/bitcoin-price-index" | html2text | grep -A 1 "Last Price" | head -n 1 | awk 'NF {print $NF}' | tr -d ',' )

	curl -s "${inspect_address_url}${address_hash}" | html2text | grep "Transacciones" -A 1 | head -n -2 | grep -v -E "Transacciones|\--" > address2.tmp
